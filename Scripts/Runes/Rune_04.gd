...
func _activate_rune():
	if !global.rune_played:
		...
		for piece in pieces:
			if piece.type == 'q' && piece.is_white != global.is_white_turn:
				piece.calculate_move_queen.connect(_on_calculate_move)
		...
		for j in range(-1,2,1):
			...
			if board.has_node(str(y) + "-" + str(x)): 
				...
				if not cell.get_child_count() == 0:
					...
					elif Piece.is_white != piece.is_white:
						piece.moves.append(Vector2i(x,y))		
				else:
					piece.moves.append(Vector2i(x,y))			
	piece.calculate_move_queen.disconnect(_on_calculate_move.bind(piece))
